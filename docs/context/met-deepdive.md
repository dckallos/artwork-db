...
| `PIPE-06` | decided | **(Session-2 review; DECIDED 2026-05-31.)** Where does the lease-claim happen — does the enrich phase open its own Snowflake connection, or is "claim" a separate command that materializes a local worklist the Mac then drains? | Today `image_enricher.py` is **Snowflake-free** (reads `_pending_object_ids()` from SQLite). The strawman's lease-MERGE (§2) requires Snowflake access in Phase 2. Fork: (a) enrich opens a Snowflake connection, claims, fetches, calls back — fewer moving parts, but couples fetch to live Snowflake; (b) a `claim` subcommand writes a local worklist file the offline fetcher drains — keeps fetch decoupled/offline-capable, but adds a command + a local artifact. **DECIDED 2026-05-31 — hybrid (owner cost concern resolved a misconception).** Owner worried an open connection during the API await + exponential backoff would hold `ARTWORK_WH` running at cost. **Correction (taught + verified):** a Snowflake session does **not** keep a warehouse running — a warehouse bills only while a query executes, plus the AUTO_SUSPEND tail. `ARTWORK_WH` is X-Small, `AUTO_SUSPEND=60s`, `AUTO_RESUME=true` (verified via `SHOW WAREHOUSES` 2026-05-31), so an idle connection during the local fetch costs nothing. Chosen pattern = **single `enrich` command, short-lived connections around a connection-free local fetch:** open→MERGE-claim N ids→close (warehouse suspends in 60s); fetch all N from the Met API locally with **no** SF connection open; open→COPY image block + MERGE-callback status→close. Gets option-(a)'s one-command simplicity + option-(b)'s zero-warehouse-during-fetch, with AUTO_SUSPEND as the safety net regardless. The lease columns + `MET_LEASE_RECLAIM_TASK` (30-min TTL) make the claim crash-safe. Ties `PIPE-03`, `PIPE-05`, `AUTO-02`. **(Phase 3 build — not yet authored; Phase 1 snapshot loader is.)** |
...
| `DATA-06` | decided | **(Session-2 review; guard BUILT 2026-05-31, not yet run.)** Does `download_csv` actually fetch the CSV, or a Git-LFS *pointer*? | `MetObjects.csv` is distributed via **Git LFS** (~225 MB, verified). `raw.githubusercontent.com/.../master/MetObjects.csv` commonly returns the ~130-byte **LFS pointer file**, not the CSV bytes. The extractor is described as "untested," so it may never have loaded 471k rows. `csv_bootstrap.download_csv` has no size/row-count integrity guard, so a pointer or truncated body would "succeed" silently. Session-3 fix: assert min-size/row-count after download, and/or switch to `media.githubusercontent.com` or the LFS media endpoint. Ties `DATA-02`. **BUILT 2026-05-31:** `csv_bootstrap.assert_real_met_csv(path)` is now the single gate both load paths (SQLite `bootstrap` + the new `snapshot_loader`) call after download/reuse: rejects the `version https://git-lfs` pointer (with remediation pointing at `media.githubusercontent.com`), enforces a 50 MB size floor, and requires `Object ID` in the header. Authored + reviewed; **not yet executed** (the 280 MB download + load runs on the Mac where the loader key lives). |
...
| ID | Status | Question | Notes / position |
|---|---|---|---|
| `AUTH-01` | **APPLIED + VERIFIED 2026-05-31** | Should `ARTWORK_LOADER_SVC` authenticate by password or key-pair (or PAT)? | **DECIDED + BUILT + APPLIED: KEY-PAIR.** Applied via `make iac` on `donkey-kong-sandbox`. `DESCRIBE USER` confirms `TYPE = SERVICE` + `PASSWORD = null` (password auth dead; key-pair is the only way in), RSA key FP `SHA256:xVLEH5…` registered; `snow connection test -c loader` = OK. **Convergence learning point:** `CREATE USER IF NOT EXISTS` was a silent no-op on the pre-existing PERSON user, so `create_service_user.sql` appends idempotent `ALTER USER … SET TYPE = SERVICE; UNSET PASSWORD; SET COMMENT` to converge (setting `TYPE=SERVICE` alone disables password login; `UNSET PASSWORD` removes the stored credential). `06_setup_loader_keypair.sh` mints `~/.snowflake/keys/loader_rsa_key.{p8,pub}`, registers the pubkey via the admin JWT connection (`git-setup/operator/register_loader_public_key.sql`), and upserts `[connections.loader]` to `authenticator=SNOWFLAKE_JWT` + `private_key_file`. `config.py`/`snowflake_uploader.py` use `private_key_file`; `.env.example` (root + met) dropped `SNOWFLAKE_PASSWORD`. Alternative C (PAT) not chosen. Ties cli-connection, `PIPE-03`. |
...
- **Loader service-user auth → KEY-PAIR: APPLIED + VERIFIED 2026-05-31.** See `AUTH-01`
  (flipped to APPLIED + VERIFIED). The empty `git-setup/operator/rotate_loader_password.sql`
  + its `06_rotate_loader_password.sh` are **deleted**; replaced by
  `06_setup_loader_keypair.sh` + `operator/register_loader_public_key.sql`.
  Applied via `make iac`; `DESCRIBE USER` shows `TYPE=SERVICE` + `PASSWORD=null`;
  `snow connection test -c loader` = OK.
...
| `create_service_user.sql` (31 ln) | matches | Key-pair migration (`AUTH-01`) is orthogonal. [Update 2026-05-31: AUTH-01 since APPLIED + VERIFIED — user now `TYPE=SERVICE`, `PASSWORD=null`; the placeholder pw + rotation comment are gone. File now 69 ln.] |
...
`DATA-06` (CSV download integrity guard), dead-code
`rename_and_update.py` removal, `AUTO-03` extraction_log writes, `PIPE-06`
lease-claim MERGE. (`AUTH-01` key-pair CLOSED + verified 2026-05-31.)
...
(next session):** data seed (control + `MET_CSV_SNAPSHOT` empty),
`DATA-01`/`DATA-06`, `AUTO-03` extraction_log writes, `PIPE-06` lease-claim MERGE,
`rename_and_update.py` removal. (`AUTH-01` key-pair CLOSED + verified 2026-05-31.)
